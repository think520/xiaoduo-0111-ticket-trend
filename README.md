...
| 文件 | 内容 |
| --- | --- |
| `output/趋势分析报告.md` | 摘要 → 数据质量 → 8 个维度 → 异常清单 → 方法与局限 → 附录 |
| `output/dashboard.html` | 自包含单文件（内联 CSS/SVG、无 JS、无 CDN），双击即可看 KPI 卡 / 5 张图 / 异常卡 / 明细表 |
| `output/metrics.json` | 全部指标与异常的结构化结果，供复核与二次开发 |
| `output/charts/*.svg` | 5 张矢量图，可直接嵌入 Markdown / PPT |
...
| 类别 | 文件 |
| --- | --- |
| 代码 | [`analyze.py`](analyze.py)（单文件 1942 行：加载/校验/指标/簇/SVG/渲染）、[`tests/test_analyze.py`](tests/test_analyze.py)（29 个用例） |
...
每个维度都必须能改变主管的某个**具体动作**，否则不做（这是取舍原则）。8 个维度的定义、理由与本数据集实测值：

| 维度 | 指标口径 | 对主管决策的价值 | 本数据实测 |
| --- | --- | --- | --- |
| **D1 时间趋势** | 每日新增量；前半段/后半段**日均**（天数不等必须用日均）；峰值日；波动系数 CV | 决定排班与人力；出现峰值日要复盘当天是否有活动/故障 | 3 → 6 条/天；前半 19 条（3.8/天）、后半 31 条（5.2/天），+36%；峰值 06-10（6 条）；CV 0.20 |
...
| **D4 处理效率** | 按分类的均值、P50、P90（nearest-rank）；SLA 超时数与超时率（仅已解决工单） | 区分"量大的问题"和"耗时的问题"：量小但耗时极长的分类会持续消耗人力并拉低满意度 | 全局均值 13.0h / P50 6h / P90 24h；**退款退货 30.0h、P90 96h**；已解决超时 4 条（9.5%），另有 5 条未解决挂起已超 SLA |
...
1. **29 个 unittest 用例**（`uv run python -m unittest discover -s tests -v` 全绿）：覆盖加载/清洗/指标/簇/异常/统计/确定性七组断言，关键数字都写成断言（如占比漂移 = +26.1pp、退款退货 P50 = 12h、超时只算已解决工单、簇成员不重复且 + 未归类 = 50）。
...
- 先用一次性脚本对 50 条数据做侦察（分布、前后半段对比、关键词聚类验证），再据此确定 8 个维度与 6 类信号；
- 编写 `analyze.py`（零依赖分析 + SVG 图表 + Markdown/HTML 渲染）与 29 个 unittest 用例；
...
- 验收：查看报告与异常清单是否可行动、复核 12 条支付工单的成员是否正确、确认 29 个测试全绿。
...
**2）开发过程**：29 个 unittest 用例全部通过（`Ran 29 tests` / `OK`）
...
> `output/dashboard.html` 是自包含单文件（无 JS/CDN），双击即可打开查看可视化总览；上表以终端截图为主，是因为采集截图时浏览器侧对本地文件 URL 有访问限制。需要 Dashboard 截图时，直接双击该文件截屏即可。
...
├─ README.md                  # 本文件
├─ docs/
│  ├─ 01-需求文档.md           # 背景/目标/8 个维度/异常定义/验收标准/待确认项
│  └─ 02-实现文档.md           # 技术选型/架构/算法口径/CLI/渲染/容错/测试
├─ tests/test_analyze.py      # 29 个验收用例
├─ screenshots/               # 6 张提交截图
